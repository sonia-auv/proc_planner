    ...
    trajectory_msgs_MultiDOFJointTrajectoryPointStruct_T *msg)
{
  ...
  geometry_msgs_TransformStruct_T t1_Transforms;
  geometry_msgs_TwistStruct_T t1_Accelerations;
  geometry_msgs_TwistStruct_T t1_Velocities;
  geometry_msgs_TransformStruct(&t1_Transforms);
  geometry_msgs_TwistStruct(&t1_Velocities);
  geometry_msgs_TwistStruct(&t1_Accelerations);
  msg->TimeFromStart = ros_DurationStruct();
  for (int i{0}; i < 44; i++) {
    msg->MessageType[i] = b_cv[i];
  }
  msg->Transforms.set_size(1);
  msg->Transforms[0] = t1_Transforms;
  msg->Velocities.set_size(1);
  msg->Velocities[0] = t1_Velocities;
  msg->Accelerations.set_size(1);
  msg->Accelerations[0] = t1_Accelerations;
